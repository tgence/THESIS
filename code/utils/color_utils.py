...
def hex_to_rgb(hexstr: str) -> Tuple[float, float, float]:
    ...
    Accepts strings like "#RRGGBB" or "RRGGBB". Returns (r, g, b) in [0, 1].
    Raises ValueError if input is not a valid 6-digit hex color.
    """
...
def relative_luminance(rgb: Tuple[float, float, float]) -> float:
    """Compute relative luminance per WCAG for an sRGB triple in [0, 1]."""
    def lin(c):
...
def hex_to_lab(hexstr: str) -> Tuple[float, float, float]:
    """Convert hex color to Lab triple (L, a, b)."""
...
def delta_e_lab(lab1, lab2) -> float:
    ...
    Accepts either tuples (L, a, b) or LabColor instances.
    """
    def to_labcolor(x):
...
def hex_to_lch(hexstr: str) -> Tuple[float, float, float]:
    """Convert hex color to LCHab triple (L, C, H)."""
    rgb = sRGBColor(*hex_to_rgb(hexstr), is_upscaled=False)
